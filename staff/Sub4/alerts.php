...
$pageTitle = "Geofence Alerts for Zones";
...
$responders = [
    ['name'=>'Responder A', 'lat'=>14.5995, 'lng'=>120.9842],
    ['name'=>'Responder B', 'lat'=>14.6100, 'lng'=>120.9900],
    ['name'=>'Responder C', 'lat'=>14.6050, 'lng'=>120.9750]
];
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <title>Geofence Alerts for Zones</title>
      ...
      <ul class="space-y-1 mt-1">
        ...
        <li>
          ...
          <ul class="max-h-0 overflow-hidden flex-col transition-all duration-500 ml-4 submodule" id="module4">
            <li><a href="./mapview.php" class="flex items-center text-sm p-1 hover:text-blue-200"><i class='bx bx-map-alt'></i><span class="ml-1">Map view of ongoing incidents</span></a></li>
            <li><a href="#" class="flex items-center text-sm p-1 hover:text-blue-200"><i class='bx bx-shield'></i><span class="ml-1">Geofence alerts for zones</span></a></li>
          </ul>
        </li>
...
.modal {
  ...
  #alerts { padding: 10px; background: #f8f8f8; max-height: 100px; overflow-y: auto; border-top: 1px solid #ccc; }
</style>
</head>
<body>
...
<script>
    ...
    // Example geofence zones
    var zones = [
        {name: "Zone 1", coords: [[14.595,120.980],[14.605,120.980],[14.605,120.990],[14.595,120.990]]},
        {name: "Zone 2", coords: [[14.600,120.970],[14.610,120.970],[14.610,120.980],[14.600,120.980]]}
    ];

    zones.forEach(function(zone) {
        L.polygon(zone.coords, {color:'blue', fillOpacity:0.2}).addTo(map)
          .bindPopup(zone.name);
    });

    // Responders markers
    var responders = <?php echo json_encode($responders); ?>;
    var responderMarkers = [];

    responders.forEach(function(resp){
        var marker = L.marker([resp.lat, resp.lng]).addTo(map)
                      .bindPopup(resp.name);
        marker.responder = resp.name;
        responderMarkers.push(marker);
    });

    // Function to check if a point is inside polygon (using turf.js)
    function checkGeofence(marker) {
        var point = turf.point([marker.getLatLng().lng, marker.getLatLng().lat]);

        zones.forEach(function(zone){
            var polygon = turf.polygon([zone.coords.map(c=>[c[1],c[0]])]); // lng, lat
            if(turf.booleanPointInPolygon(point, polygon)){
                addAlert(marker.responder + " entered " + zone.name);
            }
        });
    }
    ...
    setInterval(function(){
        responderMarkers.forEach(function(marker){
            var latOffset = (Math.random()-0.5)/1000;
            var lngOffset = (Math.random()-0.5)/1000;
            var newLat = marker.getLatLng().lat + latOffset;
            var newLng = marker.getLatLng().lng + lngOffset;
            marker.setLatLng([newLat, newLng]);
            checkGeofence(marker);
        });
    }, 3000);
