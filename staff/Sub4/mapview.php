...
$pageTitle = "Map View of Ongoing Incidents";
      ...
      <ul class="space-y-1 mt-1">
        ...
        <li>
          ...
          <ul class="max-h-0 overflow-hidden flex-col transition-all duration-500 ml-4 submodule" id="module4">
            <li><a href="#" class="flex items-center text-sm p-1 hover:text-blue-200"><i class='bx bx-map-alt'></i><span class="ml-1">Map view of ongoing incidents</span></a></li>
            <li><a href="./alerts.php" class="flex items-center text-sm p-1 hover:text-blue-200"><i class='bx bx-shield'></i><span class="ml-1">Geofence alerts for zones</span></a></li>
          </ul>
        </li>
...
// Fetch only ongoing emergency calls
$stmt = $conn->prepare("SELECT caller_name, location, status, severity, latitude, longitude FROM emergency_calls WHERE status = 'Ongoing'");
$stmt->execute();
...
<div id="map" class="flex-1 w-full rounded shadow"></div>

<script src="https://unpkg.com/leaflet/dist/leaflet.js"></script>
<script>
    var map = L.map('map').setView([14.5995, 120.9842 ], 9);

    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        attribution: '&copy; OpenStreetMap contributors'
    }).addTo(map);

    // Fetch PHP data into JS
    var ongoingCalls = <?php echo json_encode($ongoingCalls); ?>;
    ...
    ongoingCalls.forEach(function(call){
        if(call.latitude && call.longitude){ // ensure coords exist
            var icon = L.icon({
                iconUrl: 'https://cdn.jsdelivr.net/gh/pointhi/leaflet-color-markers/img/marker-icon-' + getMarkerColor(call.severity) + '.png',
            ...
            var marker = L.marker([call.latitude, call.longitude], {icon: icon}).addTo(map);

            var popupContent = "<div class='text-center'>" +
                "<b>"+call.caller_name+"</b><br/>" +
                "<b>Location:</b> " + call.location_name + "<br/>" +
                "<b>Status:</b> " + call.status + "<br/>" +
                "<b>Severity:</b> " + call.severity +
                "</div>";
