...
$pageTitle = "Heatmaps Identifying Delays";
...
$stmt = $conn->query("
    SELECT caller_name, latitude, longitude, incident_type, severity
    FROM emergency_calls
    WHERE latitude IS NOT NULL AND longitude IS NOT NULL
");
$emergencyCalls = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <title>Heatmaps Identifying Delays</title>
      ...
      <ul class="space-y-1 mt-1">
        ...
        <li>
          ...
          <ul class="max-h-0 overflow-hidden flex-col transition-all duration-500 ml-4 submodule" id="module5">
            <li><a href="./reports.php" class="flex items-center text-sm p-1 hover:text-blue-200"><i class='bx bx-bar-chart-alt'></i><span class="ml-1">Daily, weekly, monthly reports</span></a></li>
            <li><a href="#" class="flex items-center text-sm p-1 hover:text-blue-200"><i class='bx bx-map'></i><span class="ml-1">Heatmaps identifying delays</span></a></li>
          </ul>
        </li>
...
<div class="flex flex-col h-screen gap-6 p-6 bg-gray-100">
  <!-- Map Container -->
  <div id="map" class="rounded-lg shadow-lg flex-1"></div>

  <!-- Emergency Calls Table -->
  <div class="bg-white rounded-lg shadow p-6 overflow-x-auto" style="max-height: 40%;">
    <h2 class="text-xl font-semibold text-gray-700 mb-4">Emergency Call Data</h2>
    <table class="min-w-full text-sm text-left">
      ...
      <tbody>
        ...
        <tr class="border-b">
          <td class="px-4 py-2"><?= htmlspecialchars($call['caller_name']) ?></td>
          <td class="px-4 py-2"><?= htmlspecialchars($call['incident_type']) ?></td>
          <td class="px-4 py-2">
            <span class="px-2 py-1 rounded text-white 
              <?php 
                if ($call['severity'] === 'High') echo 'bg-red-500';
                elseif ($call['severity'] === 'Medium') echo 'bg-yellow-500';
                else echo 'bg-green-500';
              ?>">
              <?= htmlspecialchars($call['severity']) ?>
            </span>
          </td>
          <td class="px-4 py-2"><?= htmlspecialchars($call['latitude']) ?></td>
          <td class="px-4 py-2"><?= htmlspecialchars($call['longitude']) ?></td>
        </tr>
        <?php endforeach; ?>
      </tbody>
    </table>
  ...
  <script>
    ...
    const emergencyCalls = <?= json_encode($emergencyCalls) ?>;
    ...
    const heatData = emergencyCalls.map(call => [
      parseFloat(call.latitude),
      parseFloat(call.longitude),
      0.5
    ]);
    ...
    const icons = {
      High: L.icon({
        iconUrl: 'https://cdn-icons-png.flaticon.com/512/484/484167.png', // Red icon
        iconSize: [30, 30],
        iconAnchor: [15, 30],
        popupAnchor: [0, -28]
      }),
      Medium: L.icon({
        iconUrl: 'https://cdn-icons-png.flaticon.com/512/484/484662.png', // Orange icon
        iconSize: [30, 30],
        iconAnchor: [15, 30],
        popupAnchor: [0, -28]
      }),
      Low: L.icon({
        iconUrl: 'https://cdn-icons-png.flaticon.com/512/484/484687.png', // Green icon
        iconSize: [30, 30],
        iconAnchor: [15, 30],
        popupAnchor: [0, -28]
      })
    ...
    emergencyCalls.forEach(call => {
      const marker = L.marker([parseFloat(call.latitude), parseFloat(call.longitude)], {
        icon: icons[call.severity] || icons.Low
      }).addTo(map);

      marker.bindPopup(`
        <div>
          <h3 class="font-bold text-lg">${call.caller_name}</h3>
          <p><strong>Incident Type:</strong> ${call.incident_type}</p>
