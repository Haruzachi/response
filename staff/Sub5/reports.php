...
$pageTitle = "Daily, Weekly, Monthly reports";
...
<?php
$incidentTypes = ['Fire', 'Crime', 'Medical', 'Accident'];
$data = [];

foreach ($incidentTypes as $type) {
    $stmt = $conn->prepare("SELECT severity, COUNT(*) as count FROM emergency_calls WHERE incident_type = ? GROUP BY severity");
    $stmt->execute([$type]);
    $data[$type] = $stmt->fetchAll(PDO::FETCH_ASSOC);

    $tableStmt = $conn->prepare("SELECT caller_name, location, severity, status, created_at FROM emergency_calls WHERE incident_type = ? ORDER BY created_at DESC LIMIT 5");
    $tableStmt->execute([$type]);
    $data[$type . '_table'] = $tableStmt->fetchAll(PDO::FETCH_ASSOC);
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <title>Daily, Weekly, Monthly reports</title>
      ...
      <ul class="space-y-1 mt-1">
        ...
        <li>
          ...
          <ul class="max-h-0 overflow-hidden flex-col transition-all duration-500 ml-4 submodule" id="module5">
            <li><a href="#" class="flex items-center text-sm p-1 hover:text-blue-200"><i class='bx bx-bar-chart-alt'></i><span class="ml-1">Daily, weekly, monthly reports</span></a></li>
            <li><a href="./heatmaps.php" class="flex items-center text-sm p-1 hover:text-blue-200"><i class='bx bx-map'></i><span class="ml-1">Heatmaps identifying delays</span></a></li>
          </ul>
        </li>
...
<!---============================== DASHBOARD ==============================--->
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    <?php foreach ($incidentTypes as $type): ?>
      <div class="bg-white rounded-2xl shadow p-6">
        <h2 class="text-xl font-semibold mb-4 text-gray-700"><?= htmlspecialchars($type) ?> Incidents</h2>

        <!-- Charts Row -->
        <div class="grid grid-cols-2 gap-4 mb-6">
          <div class="bg-gray-50 rounded-lg p-3">
            <canvas id="pieChart_<?= $type ?>"></canvas>
          </div>
          <div class="bg-gray-50 rounded-lg p-3">
            <canvas id="barChart_<?= $type ?>"></canvas>
          </div>
        </div>
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full text-sm text-left">
            ...
            <tbody>
              <?php foreach ($data[$type . '_table'] as $row): ?>
                <tr class="border-b">
                  <td class="px-4 py-2"><?= htmlspecialchars($row['caller_name']) ?></td>
                  <td class="px-4 py-2"><?= htmlspecialchars($row['location']) ?></td>
                  <td class="px-4 py-2">
                    <span class="px-2 py-1 rounded text-white
                      <?php if($row['severity'] === 'High') echo 'bg-red-500';
                            elseif($row['severity'] === 'Medium') echo 'bg-yellow-500';
  ...
  <script>
    ...
    Object.keys(chartData).forEach(key => {
      if (!['Fire', 'Crime', 'Medical', 'Accident'].includes(key)) return;

      const pieCtx = document.getElementById('pieChart_' + key).getContext('2d');
      const barCtx = document.getElementById('barChart_' + key).getContext('2d');

      const severities = chartData[key].map(item => item.severity);
      const counts = chartData[key].map(item => item.count);
