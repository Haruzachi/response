...
// If no user found, logout for safety
if (!$user) {
    session_destroy();
    header("Location: ../index.php");
    exit;
...
// Determine profile image from database
$profile_image = $user['profile_image'] ?? 'blank.png';

// Correct path relative to dashboard.php
...
<head>
  ...
  <title>LGU4 Dashboard - Staff</title>
  ...
  <aside class="w-64 bg-gradient-to-b from-stone-950 to-blue-800 text-white flex flex-col h-screen">
    ...
    <nav class="flex-1 overflow-hidden px-2">
      ...
      <ul class="space-y-1 mt-1">
        <li>
          <a href="#" class="flex items-center p-2 hover:bg-blue-800 rounded transition-colors duration-200">
      ...
      <ul class="space-y-1 mt-1">
        ...
        <li>
          <button class="flex items-center w-full p-2 hover:bg-blue-800 rounded justify-between module-btn transition-colors duration-200" data-target="module7">
            <span class="flex items-center"><i class='bx bx-chat text-xl'></i><span class="ml-2">Hazard Map</span></span>
            <i class='bx bx-chevron-down transition-transform duration-300'></i>
          </button>
          <ul class="max-h-0 overflow-hidden flex-col transition-all duration-500 ml-4 submodule" id="module7">
            <li><a href="./Sub7/feedback.php" class="flex items-center text-sm p-1 hover:text-blue-200"><i class='bx bx-edit'></i><span class="ml-1">Feedback forms for responders</span></a></li>
    ...
    <div class="p-4 border-t border-blue-800">
      <ul class="space-y-1">
        ...
        <li>
          <a href="./setting.php" class="flex items-center p-2 hover:bg-blue-800 rounded">
              ...
              <table class="min-w-full divide-y divide-gray-200 border border-gray-300 rounded-lg">
                  <thead class="bg-gray-100">
                      <tr>
                          ...
                          <th class="px-4 py-2 text-left text-sm font-semibold text-gray-600">Caller Name</th>
                          <th class="px-4 py-2 text-left text-sm font-semibold text-gray-600">Incident Type</th>
                          <th class="px-4 py-2 text-left text-sm font-semibold text-gray-600">Location</th>
                          <th class="px-4 py-2 text-left text-sm font-semibold text-gray-600">Latitude</th>
                          <th class="px-4 py-2 text-left text-sm font-semibold text-gray-600">Longitude</th>
                  ...
                  <tbody class="divide-y divide-gray-100">
                      <?php if (count($emergencyCalls) > 0): ?>
                          <?php foreach ($emergencyCalls as $call): ?>
                              <tr class="hover:bg-gray-50">
                                  <td class="px-4 py-2 text-sm text-gray-700"><?= htmlspecialchars($call['id']) ?></td>
                                  <td class="px-4 py-2 text-sm text-gray-700"><?= htmlspecialchars($call['caller_name']) ?></td>
                                  <td class="px-4 py-2 text-sm text-gray-700"><?= htmlspecialchars($call['incident_type']) ?></td>
                                  <td class="px-4 py-2 text-sm text-gray-700"><?= htmlspecialchars($call['location']) ?></td>
                                  <td class="px-4 py-2 text-sm text-gray-700"><?= htmlspecialchars($call['latitude']) ?></td>
                                  <td class="px-4 py-2 text-sm text-gray-700"><?= htmlspecialchars($call['longitude']) ?></td>
                                  <td class="px-4 py-2 text-sm">
                                      <span class="px-2 py-1 rounded text-xs font-semibold 
                                          <?php if ($call['status'] === 'Pending') echo 'bg-yellow-100 text-yellow-800'; ?>
                                          <?php if ($call['status'] === 'Ongoing') echo 'bg-blue-100 text-blue-800'; ?>
                                          <?php if ($call['status'] === 'Completed') echo 'bg-green-100 text-green-800'; ?>">
                                          <?= htmlspecialchars($call['status']) ?>
                                      </span>
                                  </td>
                                  <td class="px-4 py-2 text-sm text-gray-700"><?= htmlspecialchars($call['created_at']) ?></td>
                                  <td class="px-4 py-2 text-center">
                                      <a href="./Sub4/mapview.php" 
