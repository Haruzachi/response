...
$pageTitle = "Dispatch Assignment Interface";
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['dispatch_incident'])) {
    ...
    $stmt = $conn->prepare("
        INSERT INTO dispatch_assignments (incident_id, responder_name, dispatch_status) 
        VALUES (?, ?, 'Assigned')
    ");

    header("Location: dispatch.php?dispatched=1");
    exit;
}
...
<head>
  ...
  <title>Dispatch Assignment Interface</title>
      ...
      <ul class="space-y-1 mt-1">
        ...
        <li>
          ...
          <ul class="max-h-0 overflow-hidden flex-col transition-all duration-500 ml-4 submodule" id="module2">
            ...
            <li><a href="#" class="flex items-center text-sm p-1 hover:text-blue-200"><i class='bx bx-directions'></i><span class="ml-1">Dispatch assignment interface</span></a></li>
            <li><a href="./monitoring.php" class="flex items-center text-sm p-1 hover:text-blue-200"><i class='bx bx-street-view'></i><span class="ml-1">Monitoring of incidents</span></a></li>
          </ul>
        </li>
...
<div class="container mx-auto mt-8 p-6 bg-white rounded shadow-lg">
    ...
    <?php if (isset($_GET['dispatched'])): ?>
        <div class="bg-green-100 text-green-800 p-3 rounded mb-4">Incident dispatched successfully!</div>
    <?php endif; ?>
        ...
        <tbody>
            <?php foreach ($calls as $call): ?>
                <tr>
                    ...
                    <td class="p-2 border">
                        <form method="POST" action="dispatch.php" class="flex space-x-2">
                            <input type="hidden" name="incident_id" value="<?= $call['id'] ?>">
                            
                            <select name="status" class="p-1 border rounded" required>
                                <option value="Pending" <?= $call['status'] === 'Pending' ? 'selected' : '' ?>>Pending</option>
                                <option value="Ongoing" <?= $call['status'] === 'Ongoing' ? 'selected' : '' ?>>Ongoing</option>
                            </select>

                            <button type="submit" name="dispatch_incident" class="bg-blue-500 hover:bg-blue-600 text-white px-3 rounded">Update</button>
                        </form>
                    </td>
                </tr>
            <?php endforeach; ?>
