...
$pageTitle = "Monitoring of Incidents";
...
function getSeverityBadge($severity) {
    switch ($severity) {
        case 'Low': return '<span class="bg-green-200 text-green-800 px-2 py-1 rounded">Low</span>';
        case 'Medium': return '<span class="bg-yellow-200 text-yellow-800 px-2 py-1 rounded">Medium</span>';
        case 'High': return '<span class="bg-orange-200 text-orange-800 px-2 py-1 rounded">High</span>';
        case 'Critical': return '<span class="bg-red-200 text-red-800 px-2 py-1 rounded">Critical</span>';
        default: return '<span class="bg-gray-200 text-gray-800 px-2 py-1 rounded">Unknown</span>';
    }
}

$defaultLat = 12.8797;  // Approx center of the Philippines
$defaultLng = 121.7740;
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <title>Monitoring of Incidents</title>
      ...
      <ul class="space-y-1 mt-1">
        ...
        <li>
          ...
          <ul class="max-h-0 overflow-hidden flex-col transition-all duration-500 ml-4 submodule" id="module2">
            <li><a href="./severity.php" class="flex items-center text-sm p-1 hover:text-blue-200"><i class='bx bx-time-five'></i><span class="ml-1">Severity assessment tool</span></a></li>
            <li><a href="./dispatch.php" class="flex items-center text-sm p-1 hover:text-blue-200"><i class='bx bx-directions'></i><span class="ml-1">Dispatch assignment interface</span></a></li>
            <li><a href="#" class="flex items-center text-sm p-1 hover:text-blue-200"><i class='bx bx-street-view'></i><span class="ml-1">Monitoring of incidents</span></a></li>
          </ul>
        </li>
...
<div class="container mx-auto mt-8 p-6 bg-white shadow-lg rounded-lg">
    ...
    <!-- INCIDENT LIST TABLE -->
    <h3 class="text-xl font-semibold mt-6 mb-3">Incident List</h3>
    <table class="min-w-full border border-gray-300">
        <thead>
            <tr class="bg-gray-200">
                <th class="p-2 border">ID</th>
                <th class="p-2 border">Caller</th>
                <th class="p-2 border">Incident Type</th>
        ...
        <?php foreach ($calls as $call): ?>
            <tr>
                <td class="p-2 border"><?= htmlspecialchars($call['id']) ?></td>
                <td class="p-2 border"><?= htmlspecialchars($call['caller_name']) ?></td>
                <td class="p-2 border"><?= htmlspecialchars($call['incident_type']) ?></td>
                <td class="p-2 border"><?= htmlspecialchars($call['location']) ?></td>
                <td class="p-2 border"><?= getSeverityBadge($call['severity']) ?></td>
                <td class="p-2 border"><?= htmlspecialchars($call['status']) ?></td>
                <td class="p-2 border"><?= htmlspecialchars($call['created_at']) ?></td>
            </tr>
        <?php endforeach; ?>
        </tbody>
    </table>
...
<script>
    ...
    // Define approximate bounding box for the Philippines
    const philippinesBounds = {
        latMin: 4.2158,   // Southernmost (Tawi-Tawi)
        latMax: 21.3210,  // Northernmost (Batanes)
        lngMin: 116.9346, // Westernmost (Palawan)
        lngMax: 126.6040  // Easternmost (Philippine Sea)
    };
    ...
    var incidents = <?php echo json_encode($calls); ?>;

    // Add markers only if within the Philippines bounds
    incidents.forEach(function(incident) {
        if (incident.latitude && incident.longitude) {
            ...
            if (lat >= philippinesBounds.latMin && lat <= philippinesBounds.latMax &&
                lng >= philippinesBounds.lngMin && lng <= philippinesBounds.lngMax) {

                var markerIcon = icons[incident.incident_type] || icons["Default"];
                var marker = L.marker([lat, lng], { icon: markerIcon }).addTo(map);

                var popupContent = `
                    <strong>Incident #${incident.id}</strong><br>
                    <b>Type:</b> ${incident.incident_type}<br>
                    <b>Location:</b> ${incident.location}<br>
                    <b>Severity:</b> ${incident.severity}<br>
                    <b>Status:</b> ${incident.status}<br>
                    <b>Reported:</b> ${incident.created_at}
                `;
