...
$pageTitle = "Severity Assessment Tool";
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['update_severity'])) {
    $incidentId = $_POST['incident_id'];
    $severity = $_POST['severity'];

    $stmt = $conn->prepare("UPDATE emergency_calls SET severity = ? WHERE id = ?");
    $stmt->execute([$severity, $incidentId]);

    header("Location: dispatch.php");
exit;
}

$stmt = $conn->prepare("SELECT * FROM emergency_calls ORDER BY created_at DESC");
$stmt->execute();
...
<head>
  ...
  <title>Severity Assessment Tool</title>
      ...
      <ul class="space-y-1 mt-1">
        ...
        <li>
          ...
          <ul class="max-h-0 overflow-hidden flex-col transition-all duration-500 ml-4 submodule" id="module2">
            <li><a href="#" class="flex items-center text-sm p-1 hover:text-blue-200"><i class='bx bx-time-five'></i><span class="ml-1">Severity assessment tool</span></a></li>
...
<div class="container mx-auto mt-8 p-6 bg-white shadow-lg rounded-lg">
    ...
    <?php if (isset($_GET['updated'])): ?>
        <div class="bg-green-100 text-green-800 p-3 rounded mb-4">Severity updated successfully!</div>
    <?php endif; ?>
    ...
    <table class="min-w-full border border-gray-300">
        <thead>
            <tr class="bg-gray-200">
                ...
                <th class="p-2 border">Caller Name</th>
        ...
        <?php foreach ($calls as $call): ?>
            <tr>
                ...
                <td class="p-2 border">
                    <form method="POST" action="severity.php" class="flex space-x-2">
                        <input type="hidden" name="incident_id" value="<?= $call['id'] ?>">
                        <select name="severity" class="p-1 border rounded">
                            <option value="Low" <?= $call['severity'] === 'Low' ? 'selected' : '' ?>>Low</option>
                            <option value="Medium" <?= $call['severity'] === 'Medium' ? 'selected' : '' ?>>Medium</option>
                            <option value="High" <?= $call['severity'] === 'High' ? 'selected' : '' ?>>High</option>
                            <option value="Critical" <?= $call['severity'] === 'Critical' ? 'selected' : '' ?>>Critical</option>
                        </select>
                        <button type="submit" name="update_severity" class="bg-blue-500 hover:bg-blue-600 text-white px-3 rounded">Save</button>
                    </form>
                </td>
