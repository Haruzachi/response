...
$pageTitle = "Shared Communication";
...
<head>
  ...
  <title>Shared Communication</title>
      ...
      <ul class="space-y-1 mt-1">
        ...
        <li>
          ...
          <ul class="max-h-0 overflow-hidden flex-col transition-all duration-500 ml-4 submodule" id="module6">
            <li><a href="#" class="flex items-center text-sm p-1 hover:text-blue-200"><i class='bx bx-chat'></i><span class="ml-1">Shared communication</span></a></li>
          </ul>
        </li>
...
// Fetch messages grouped by department
$departments = ['Fire', 'Police', 'Hospital'];
$messages_by_dept = [];
foreach ($departments as $dept) {
    ...
    ");
    $query->execute([$dept]);
    $messages_by_dept[$dept] = $query->fetchAll(PDO::FETCH_ASSOC);
}
?>
...
<div class="max-w-6xl mx-auto p-6">
  ...
  <!-- New Message Form -->
  <form action="add_message.php" method="POST" class="mb-8 bg-white p-4 rounded shadow grid grid-cols-1 md:grid-cols-2 gap-4">
    <textarea name="message" rows="2" required
      placeholder="Type your message..." 
      class="w-full border rounded p-2 focus:ring-2 focus:ring-blue-500"></textarea>
    
    <select name="department" required class="border rounded p-2">
    ...
    <button type="submit" 
      class="md:col-span-2 bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
      Send Message
    </button>
  </form>
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    <?php foreach ($departments as $dept): ?>
      <div class="bg-white p-4 rounded shadow h-[500px] flex flex-col">
        <h2 class="font-bold text-xl mb-4 text-center text-blue-800"><?= $dept ?> Department</h2>
        <div class="flex-1 overflow-y-auto space-y-3">
          <?php if (empty($messages_by_dept[$dept])): ?>
            <div class="text-gray-500 text-center">No messages yet.</div>
          <?php else: ?>
            <?php foreach ($messages_by_dept[$dept] as $msg): ?>
              <div class="p-3 bg-gray-50 border-l-4 border-blue-600 rounded shadow-sm">
