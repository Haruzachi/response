...
$pageTitle = "Manual Override for Dispatchers";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['override_unit'])) {
    $call_id = $_POST['call_id'];
    $new_unit_id = $_POST['unit_id'];

    $prevAllocation = $conn->prepare("SELECT * FROM resource_allocations WHERE emergency_call_id=?");
    $prevAllocation->execute([$call_id]);
    $prev = $prevAllocation->fetch(PDO::FETCH_ASSOC);

    if($prev) {
     
        $conn->prepare("UPDATE resources SET status='Available' WHERE id=?")->execute([$prev['resource_id']]);

        $conn->prepare("UPDATE resource_allocations SET resource_id=?, assigned_at=NOW() WHERE id=?")
             ->execute([$new_unit_id, $prev['id']]);

        $conn->prepare("UPDATE resources SET status='Dispatched' WHERE id=?")->execute([$new_unit_id]);

        $successMessage = "✅ Unit overridden successfully!";
    }
}

$sql = "
SELECT ec.*, r.unit_name, r.resource_type
FROM emergency_calls ec
LEFT JOIN resource_allocations ra ON ec.id = ra.emergency_call_id
LEFT JOIN resources r ON ra.resource_id = r.id
WHERE ec.status='Ongoing'
ORDER BY ec.created_at DESC
";
$emergencyCalls = $conn->query($sql)->fetchAll(PDO::FETCH_ASSOC);

$resources = $conn->query("SELECT * FROM resources WHERE status='Available'")->fetchAll(PDO::FETCH_ASSOC);
?>
...
<head>
  ...
  <title>Manual Override for Dispatchers</title>
      ...
      <ul class="space-y-1 mt-1">
        ...
        <li>
          ...
          <ul class="max-h-0 overflow-hidden flex-col transition-all duration-500 ml-4 submodule" id="module3">
            <li><a href="./availables.php" class="flex items-center text-sm p-1 hover:text-blue-200"><i class='bx bx-car'></i><span class="ml-1">Availables</span></a></li>
            <li><a href="#" class="flex items-center text-sm p-1 hover:text-blue-200"><i class='bx bx-refresh'></i><span class="ml-1">Manual override for dispatchers</span></a></li>
            <li><a href="./reports.php" class="flex items-center text-sm p-1 hover:text-blue-200"><i class='bx bx-bar-chart'></i><span class="ml-1">Reports on resource utilization</span></a></li>
          </ul>
        </li>
...
<div class="container mx-auto bg-white p-6 rounded shadow">
    <h1 class="text-2xl font-bold mb-4">Override Units</h1>
    ...
    <table class="w-full border mb-4">
        <thead class="bg-gray-200">
            <tr>
                <th class="border p-2">Caller Name</th>
                <th class="border p-2">Incident Type</th>
                <th class="border p-2">Location</th>
                <th class="border p-2">Assigned Unit</th>
                <th class="border p-2">Override Unit</th>
            </tr>
        </thead>
        <tbody>
            <?php foreach($emergencyCalls as $call): ?>
            <tr>
                <td class="border p-2"><?= htmlspecialchars($call['caller_name']) ?></td>
                <td class="border p-2"><?= htmlspecialchars($call['incident_type']) ?></td>
                <td class="border p-2"><?= htmlspecialchars($call['location']) ?></td>
                <td class="border p-2">
                    <?= $call['unit_name'] ? htmlspecialchars($call['unit_name']) . " (" . htmlspecialchars($call['resource_type']) . ")" : "Not assigned" ?>
                </td>
                <td class="border p-2">
                    <form method="POST" class="flex gap-2">
                        <input type="hidden" name="call_id" value="<?= $call['id'] ?>">
                        <select name="unit_id" class="border p-1 rounded" required>
                            <option value="">-- Select Unit --</option>
                            <?php
                                $incidentType = strtolower($call['incident_type']);
                                foreach($resources as $unit) {
                                    $unitType = strtolower($unit['resource_type']);

                                    // Map incident types to unit types
                                    $map = [
                                        'fire' => 'fire',
                                        'police' => 'police',
                                        'accident' => 'ems',
                                    ...
                                    if(isset($map[$incidentType]) && $map[$incidentType] === $unitType) {
                                        echo '<option value="'.$unit['id'].'">'.htmlspecialchars($unit['unit_name']).' ('.htmlspecialchars($unit['resource_type']).')</option>';
                                    }
                                }
                            ?>
                        </select>
                        <button type="submit" name="override_unit" class="bg-orange-500 text-white px-2 py-1 rounded">Override</button>
                    </form>
                </td>
            </tr>
            <?php endforeach; ?>
