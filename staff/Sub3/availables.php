...
$emergencyCalls = $conn->query("SELECT * FROM emergency_calls WHERE status='Ongoing' ORDER BY created_at DESC")->fetchAll(PDO::FETCH_ASSOC);

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['assign_unit'])) {
    $call_id = $_POST['call_id'];
    $unit_id = $_POST['unit_id'];

    $stmt = $conn->prepare("INSERT INTO resource_allocations (emergency_call_id, resource_id) VALUES (?, ?)");
    $stmt->execute([$call_id, $unit_id]);

    $conn->prepare("UPDATE resources SET status='Dispatched' WHERE id=?")->execute([$unit_id]);

    $conn->prepare("UPDATE emergency_calls SET status='Ongoing' WHERE id=?")->execute([$call_id]);

    $successMessage = "✅ Unit assigned successfully!";
}
      ...
      <ul class="space-y-1 mt-1">
        ...
        <li>
          ...
          <ul class="max-h-0 overflow-hidden flex-col transition-all duration-500 ml-4 submodule" id="module3">
            <li><a href="#" class="flex items-center text-sm p-1 hover:text-blue-200"><i class='bx bx-car'></i><span class="ml-1">Availables</span></a></li>
            <li><a href="./override.php" class="flex items-center text-sm p-1 hover:text-blue-200"><i class='bx bx-refresh'></i><span class="ml-1">Manual override for dispatchers</span></a></li>
...
function filterUnits() {
    const call = document.getElementById('call_id');
    const unit = document.getElementById('unit_id');
    const type = call.options[call.selectedIndex].dataset.type;

    for (let i = 0; i < unit.options.length; i++) {
        unit.options[i].style.display = (unit.options[i].dataset.type === type) ? '' : 'none';
    }
}
</script>
</head>
...
<h1 class="text-2xl font-bold mb-4">Assign Units</h1>
...
<select name="call_id" id="call_id" onchange="filterUnits()" class="border p-2 rounded" required>
<option value="">-- Select Emergency Call --</option>
<?php foreach($emergencyCalls as $call): ?>
<option value="<?= $call['id'] ?>" data-type="<?= $call['incident_type'] ?>">
<?= htmlspecialchars($call['caller_name']) ?> | <?= htmlspecialchars($call['incident_type']) ?> | <?= htmlspecialchars($call['location']) ?>
</option>
<?php endforeach; ?>
</select>

<select name="unit_id" id="unit_id" class="border p-2 rounded" required>
<option value="">-- Select Unit --</option>
<?php foreach($resources as $unit): ?>
<option value="<?= $unit['id'] ?>" data-type="<?= $unit['resource_type'] ?>">
<?= htmlspecialchars($unit['unit_name']) ?> (<?= htmlspecialchars($unit['resource_type']) ?>)
</option>
<?php endforeach; ?>
</select>

<button type="submit" name="assign_unit" class="bg-blue-500 text-white px-4 py-2 rounded">Assign</button>
</form>
</div>
