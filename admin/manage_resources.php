...
//______________________________________________//
try {
    $stmt = $conn->query("SELECT id, unit_name, resource_type, status, current_location FROM resources ORDER BY id DESC");
          ...
          <tbody>
            <?php if (!empty($resources)): ?>
              ...
              <tr class="border-t hover:bg-gray-50">
                <td class="px-4 py-2"><?= htmlspecialchars($res['id']) ?></td>
                <td class="px-4 py-2"><?= htmlspecialchars($res['unit_name']) ?></td>
                <td class="px-4 py-2"><?= htmlspecialchars($res['resource_type']) ?></td>
                <td class="px-4 py-2"><?= htmlspecialchars($res['status']) ?></td>
                <td class="px-4 py-2"><?= htmlspecialchars($res['current_location']) ?></td>
                <td class="px-4 py-2 text-center">
                  <a href="edit_resource.php?id=<?= $res['id'] ?>" class="text-blue-500 hover:underline">Edit</a> |
                  <a href="delete_resource.php?id=<?= $res['id'] ?>" onclick="return confirm('Are you sure?')" class="text-red-500 hover:underline">Delete</a>
                </td>
              </tr>
              <?php endforeach; ?>
            <?php else: ?>
              <tr>
                <td colspan="6" class="text-center py-4 text-gray-500">No resources found.</td>
              </tr>
            <?php endif; ?>
          </tbody>
        </table>
